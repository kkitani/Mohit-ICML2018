...
To avoid repeatedly solving the MDP in max-margin formulations, \cite{klein2012inverse} use expert policy estimates to cast an IRL problem, with a linearly parameterized reward, into a supervised multi-class classification problem and call it Structured Classification IRL (SCIRL). Reducing the IRL problem to a classification problem allows one to avoid solving the MDP. Since the expert policy estimates in SCIRL are the same as CCP values used in our CCP-IRL, SCIRL is similar in spirit to our work. However, a key restriction in SCIRL is the \emph{linear} parameterization of the reward function\footnote{\cite{klein2012inverse} suggest using basis functions to generate flexibility, however, this introduces another layer of approximation and is not generalizable to non-linear reward functions.}. In contrast, CCP-IRL does not involve such assumptions, and can be used to extract both parameterized (linear and non-linear) as well as non-parameterized reward functions. Additionally, since SCIRL solves a max-margin problem, it suffers from theoretical difficulties when learning under observations of suboptimal  behavior, as discussed in \cite{ziebart2010modeling}. Alternately, since CCP-IRL is constrained to match demonstrated feature expectations, it is guaranteed to match the performance of demonstrated behavior even if it is sub-optimal.

One work that avoids solving the MDP repeatedly in MaxEnt-IRL is Relative Entropy IRL (RelEntIRL) \cite{boularias2011relative}. RelEntIRL uses a sampling based approach, by following an arbitrary policy, to estimate the partition function. However, this still requires sampling trajectories according to the policy being optimal for the current reward, which involves solving the MDP. To avoid this, RelEntIRL uses importance sampling between a baseline policy and the arbitrary sampling policy. However, for equivalence with maximum entropy methods, the baseline policy needs to be uniform, which reduces the method to feature matching and may not be suitable for complex MDPs \cite{finn2016guided}. Further, RelEntIRL can also suffer from the high variance associated with importance sampling based methods \cite{hammersley1964general}.
In contrast, CCP-IRL avoids solving the MDP problem by using an efficient representation of the value function. This allows CCP-IRL to avoid sampling from an arbitrary policy and avoids the high variance associated with sampling based techniques.

Alternately, another method that avoids solving the MDP problem focuses on linearly solvable MDP problems \cite{todorov2007linearly}. Dvijotham and Todorov \yrcite{dvijotham2010inverse} present an efficient IRL algorithm, which they call OptV, for such linearly solvable MDPs. However, this new class of MDPs assume that the Bellman equation can be transformed into a linear equation. This is a large approximation over the traditional MDP formulation.
Also, since OptV uses value-function parameterization instead of the usual cost-function parameterization, it suffers from generalization difficulties when the value-function parameters are transferred across environments \cite{ziebart_phd, levine2012continuous}.
