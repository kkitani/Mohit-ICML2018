...
In particular, we describe an algorithm to solve the IRL problem, using easy-to-compute estimates of the Conditional Choice Probability (CCP)
vector, which is the policy function of an expert integrated over factors econometricians do not observe. Using the language of structural econometrics, we re-frame the optimal decision problem and introduce an alternative representation of differences in conditional value functions due to \cite{hotz}, based on CCPs. Further, the use of CCPs also reduces the computational cost of solving the IRL problem. Specifically, under the CCP representation, we show how one can avoid repeated calls to the dynamic programming subroutine typically used in IRL methods. We show, via extensive experimentation on standard IRL benchmarks that CCP-IRL is able to outperform state-of-the-art methods, with as much as a 10$\times$ speedup and without compromising on the quality of the recovered reward function.
\end{abstract} 
...
%\footnote{\cite{klein2012inverse} derive an alternative framework that also avoids repeated solution of the DP. However, they do so in a max-margin planning setting and are restricted to linearly parameterized reward functions only. We discuss specific differences in later sections.
%The CCP framework provides greater flexibilty and does not rely on linearity to reduce computation. We also discuss specific differences between our approach later on.
%}
%As shown by \cite{magnac}, the CCP-IRL framework also enables us to estimate the reward function non-parametrically. We derive this result in the appendix.\footnote{\cite{klein2012inverse} derive an alternative framework that also avoids repeated solution of the DP. However, they are restricted to considering reward functions that can be expressed in linear form. The CCP framework provides greater flexibilty and does not rely on linearity to reduce computation.} Moreover, there are further benefits when considering non-stationary environments, discussed in \cite{arcidiacono2014nonstationary}.
...
%Further, this speedup holds across large state spaces and increases for complex problems, such as, problems where value iteration takes much longer to converge. 
...
Our contributions in this paper include, 1) We make an important connection between the IRL literature in economics and robotics. Specifically, we show the equivalence between the maximum causal entropy approach of \cite{ziebart2010modeling} and conditional choice probability estimation under Type 1 Extreme value assumption of \cite{rust_gmc}. 2) We introduce the CCP literature from economics into robotics and propose a new IRL algorithm called CCP-IRL. 3) Finally, we discuss the generalizability of the above CCP approach, including its usage in non-parameteric reward estimation, solving the under-identifiability problem in IRL \emph{etc.} and open up new vistas to important areas in IRL research.
%==========================================================%
...
%For instance, it is not clear how max-margin formulations can be extended to continuous action spaces.
%Alternately, applying probabilistic approaches, such as Max-Ent IRL \cite{ziebart}, to continuous spaces involves its own set of assumptions.
...
To avoid repeatedly solving the MDP in max-margin formulations, \cite{klein2012inverse} use expert policy estimates to cast an IRL problem, with a linearly parameterized reward, into a supervised multi-class classification problem and call it Structured Classification IRL (SCIRL). Reducing the IRL problem to a classification problem allows one to avoid solving the MDP. Since the expert policy estimates in SCIRL are the same as CCP values used in our CCP-IRL, SCIRL is similar in spirit to our work. However, a key restriction in SCIRL is the \emph{linear} parameterization of the reward function\footnote{\cite{klein2012inverse} suggest using basis functions to generate flexibility, however, this introduces another layer of approximation and is not generalizable to non-linear reward functions.}. In contrast, CCP-IRL does not involve such assumptions, and can be used to extract both parameterized (linear and non-linear) as well as non-parameterized reward functions. Additionally, since SCIRL solves a max-margin problem, it suffers from theoretical difficulties when learning under observations of suboptimal  behavior, as discussed in \cite{ziebart2010modeling}. Alternately, since CCP-IRL involves explicit feature matching constraints, it will match the performance of demonstrated behavior, even if it is sub-optimal.

One work that avoids solving the MDP repeatedly under maximum entropy distribution is Relative Entropy IRL (RelEntIRL) \cite{boularias2011relative}. RelEntIRL uses a sampling based approach, by following an arbitrary policy, to estimate the value function. However, this still requires sampling trajectories according to the optimal policy for the current reward, which involves solving the MDP. To avoid this, RelEntIRL uses importance sampling between a baseline policy and the arbitrary sampling policy. However, such baseline policies are in general not known \cite{ziebart_phd}, and hence RelEnt-IRL can only be applied when appropriate baseline policies are known. Further, RelEntIRL can also suffer from the high variance associated with importance sampling based methods \cite{hammersley1964general}.
In contrast, CCP-IRL avoids solving the MDP problem by using an efficient representation of the value function. This allows CCP-IRL to avoid sampling from an arbitrary policy and also avoid the high variance associated with sampling based techniques.

Alternately, another method that avoids solving the MDP problem focuses on linearly solvable MDP problems \cite{todorov2007linearly}. \cite{dvijotham2010inverse} present an efficient IRL algorithm, which they call OptV, for such linearly solvable MDPs. However, this new class of MDPs assumes that the Bellman equation can be transformed into a linear equation. This linearization assumption is a large approximation over the traditional MDP formulation. Also, since OptV uses value-function parameterization instead of the usual cost-function parameterization, it suffers from generalization difficulties when the value-function parameters are transferred across environments\cite{ziebart_phd, levine2012continuous}.

% To scale Max-Ent IRL \cite{ziebart} to large spaces, past work \cite{huang2015approximate} has focused on approximating the value iteration step in IRL. However, since the MDP problem is still being solved (although approximately) repeatedly, this does not scale to very large spaces. For continuous domains, most algorithms use local approximation to estimate the partition function in MaxEnt-IRL. Using local approximations requires giving up on global optimality, but allows the algorithms to scale to high dimensional continuous domains \cite{levine2012continuous, kalakrishnan2013learning, finn2016guided}. Although, the CCP approach can be applied to continuous control settings \cite{altuug1998effect}, it is not yet clear how it can be extended to high dimensional control spaces in robotics. It would be interesting to see how the CCP approach can be combined with the above approximations in continuous spaces.

% Other work \cite{levine2012continuous, kalakrishnan2013learning, finn2016guided} has also used local approximation to estimate the partition function in MaxEnt-IRL. Using local approximations requires giving up on global optimality, but allows the algorithms to scale to high dimensional continuous domains. In contrast, the CCP approach has also been applied to continuous control settings \cite{altuug1998effect}. However, such approaches are beyond the scope of our initial work.

In contrast to previous works, our CCP-IRL algorithm does not involve any assumption on the MDP framework, and is applicable to both parameterized (linear and non-linear) and non-parameterized reward function formulations. Moreover, the CCP approach is applicable to situations with continuous state or action spaces \cite{altuug1998effect}. There has also been progress in applying CCP-like estimators to continuous controls in complex environments, such as dynamic strategic settings \cite{pesendorfer}. However, in this preliminary work we focus on discrete problem settings to establish the fundamental connections between economics and robotics literature, and leave a discussion of continuous extensions for future work.
...
% We assume a discrete state space, to avoid technical machinery out of the scope of this paper, although this is not strictly necessary \cite{hansen1982generalized}.
...
\section{Conditional Choice Probability - Inverse Reinforcement Learning} \label{ccp_irl}
...
%Although, from a computational perspective sometimes estimating the inverse matrix can be fast itself, Zeibart \yrcite{ziebart_phd} discusses some of these considerations.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experiments} \label{experiments}

To understand the strengths and weaknesses of our proposed CCP-IRL technique, we examine the performance of our method along three dimensions: (1) computational efficiency: How fast is CCP-IRL? (2) data requirements: How much data is needed to obtain reliable CCP estimates? and (3) generalizability: How well does CCP-IRL handle various forms of reward functions?


%To examine the computational efficiency of CCP-IRL we experiment on multiple benchmark IRL problems. Empirical evidence shows that CCP-IRL is able to match expert performance with \textit{much less} computational time when compared to MaxEnt-IRL. Further, as noted before, we also examine the underlying assumptions of consistent CCP estimates. For this, we see how CCP-IRL's performance is affected by the number of expert demonstrations.

% To validate our approach we illustrate, (1) the computational efficiency of CCP-IRL and (2) the underlying assumptions of consistent CCP estimates \emph{i.e.,} we show the data requirement for CCP-IRL. To verify the computational improvement using CCP-IRL, we compare the CPU time. We show that our algorithm is able to achieve similar qualitative and quantitative performance with \textit{much less} computational time. Additionally, to verify the assumptions of consistent CCP estimates we see how CCP-IRL is affected by the number of expert demonstrations.

%To show the generalizability of CCP-IRL to both linear and non-linear parameterizations we experiment with both of these formulations. For each formulation we use benchmark tasks used previously in the IRL literature \cite{klein2012inverse, Levine2013}. For the former, similar to previous works \cite{abbeel2004apprenticeship, klein2012inverse}, we use a car driving simulator. While for the latter, we use the Objectworld task as described in \cite{Levine2013}. 

\noindent{\textbf{Baselines:}} As discussed earlier CCP-IRL is closely related to MaxEnt-IRL as they have identical objective functions (though they are derived from two different perspectives). We use MaxEnt-IRL as the primary IRL baseline algorithm for comparision. For our secondary baseline algorithm, we compare against SC-IRL which can be interpreted to be a max-margin IRL algorithm that makes use of CCP estimates.

%We also use SC-IRL as another baseline.Since SCIRL also uses expert CCP estimates, although in a max-margin formulation, we use both SC-IRL and CCP-IRL to show the efficacy of using CCP estimates in an IRL framework. Further, we also use SCIRL to highlight the generalizability of CCP-IRL to non-linear reward formulations, 

\noindent{\textbf{Metrics:}} For comparative analysis, we use the negative log likelihood (NLL) \cite{kitani2012activity} and the expected value difference (EVD) \cite{levine2011nonlinear}. NLL is a probabilistic comparison metric and evaluates the likelihood of a path under the predicted policy. For a policy $\pi$, NLL is defined as, $NLL(\pi) = E_{\pi(a|s)}\left[-\log \prod_{t} \pi(a_{t}|s_{t}) \right]$. In contrast, EVD directly measures the difference between the optimal and learned policy by comparing the value function values obtained with each policy under the \textit{true reward} distribution. 


\subsection{Highway Driving: Linear Reward Function} 

We begin by examining an IRL problem which has the simplest form of reward function -- a linear function of state features. Under this linear reward function, we evaluate (1) the computational speed of our CCP-IRL algorithm and (2) the effect of the amount of data on the quality of the recovered reward function.

%First we look at the computational efficiency of CCP-IRL in environments where the reward is a linear function of the state features. We also examine CCP-IRL's assumption of consistent CCP estimates.
...
Similar to previous works \cite{abbeel2004apprenticeship, klein2012inverse}, we use a car driving simulator for the linear reward function setting. The optimal policy in this environment is to drive on a multi-lane highway while avoiding collisions with other cars. The agent's car can either move left or right, accelerate, decelerate or keep a constant speed. The underlying linear reward function is defined as a linear function of staying on the road, avoiding collisions and driving with high speed.
Following Klein \emph{et al.} \yrcite{klein2012inverse}, we parameterize the state space using 9 horizontal positions for the user's car, 3 horizontal and 9 vertical positions for the closest traffic's car and 3 speeds. This yields a state feature vector of size $729$. For experiments with MDPs with large state space, we increase the granularity of each of the quantization bins, such as, car speed, number of lanes \emph{etc.}. However, the one-hot feature representation used in \cite{klein2012inverse}, does not scale well to extremely large state spaces (\emph{e.g.,} millions of states). Hence, for very large state spaces, we group adjacent lanes and closest traffic car's position into one feature, to keep the feature vector representation compact.
...
\begin{tabular}{@{}llll@{}}
\toprule
                |S|  &     MaxEnt (sec)   & CCP (sec)          &   Speedup             \\ \midrule
         ...
         $10^6$       &     197399.03      & \textbf{16982.17}  &   $\mathbf{12}\times$ \\ \bottomrule
\end{tabular}
...
First, we evaluate the computational speed of CCP-IRL against MaxEnt-IRL over differing sizes of the state space ($10^3$ to $10^6$). A summary of computational speed (CPU usage in seconds) is given in Table~\ref{table:table_results_macro_cells}. For the IRL problem with only 1000 states, our method provides a 9$\times$ speed up. For the largest state space of 1 million, our CCP-IRL is 12$\times$ faster than MaxEnt-IRL. The pre-computation of average feature counts (and thus the removal of repeated calls to a DP algorithm) utilized by our CCP-IRL method gives a significant speed advantage over MaxEnt-IRL, especially as the state space grows larger.




Next we analyze how the amount of data effects the performance of CCP-IRL.
Figure \ref{fig:img_maxent_vs_ccp_gridworld_macro_cell} plots the NLL and EVD as a function of increasing the amount of data (\emph{i.e.,} number of expert trajectories). When the amount of trajectories is limited (less than 20) CCP- IRL shows poor performance as only a few CCP estimates are observed.
However, with a moderate number of trajectories ($\sim$30) CCP-IRL is able to recover a reward function which can be used to mimic expert behavior well. When compared against MaxEnt IRL, CCP-IRL does require a slightly larger number of trajectories. This is expected as CCP-IRL requires sufficient samples from the expert policy to obtain reliable CCP estimates. Interestingly, we observe that SC-IRL is able to obtain a better NLL and EVD score when only a limited number (less than 20) of expert trajectories are available. We believe that when the reward function is linear, it may be easier to solve the supervised learning problem instead of solving the reinforcement learning problem. However, the more important observation is that the use of CCP estimates, whether in a MaxEnt approach or Max-Margin approach can lead to algorithms with low sample complexity.




%However, Figure \ref{fig:img_maxent_vs_ccp_gridworld_macro_cell} also shows that the sample complexity for CCP-IRL is not significantly worse than MaxEnt-IRL. Interestingly, SCIRL requires less data compared to both MaxEnt-IRL and CCP-IRL. 

%We believe this is because SCIRL is solving a supervised classification problem to directly imitate the expert, and such supervised learning provide better learning guarantees than RL based approaches \cite{beygelzimer2011contextual}. However, the low sample complexity of both SCIRL and CCP-IRL reflects the efficacy of CCPs in an IRL setting.

\begin{figure}[tb]
\centering
  \begin{tabular}{cc}
    \MSHangBox{\includegraphics[width=0.22\textwidth]{images/highway/medium_mdp_smooth/test_nll_algo_3.pdf}}&
    \MSHangBox{\includegraphics[width=0.22\textwidth]{images/highway/medium_mdp_smooth/test_evd_algo_3.pdf}}
    \end{tabular}
    \caption{Results for Highway MDP with 5,000 states. Left: Minimum NLL results with varying number of trajectories. Right: Expected Value Difference results. Each expert trajectory was of length 30.}
...
\subsection{Objectworld: Non-Linear Reward Function}

To show the generalizability of CCP-IRL we now examine problems where reward is a non-linear function of state features. For this, similar to previous works we use the Objectworld environment \cite{levine2011nonlinear}.
...
% We would like to note that although such an approach can be used to approximate non-linear rewards, it requires significant hand-tuning and is not generalizable to more complex reward scenarios. 
...
\begin{tabular}{@{}llll@{}}
\hline
Experiment & MaxEnt & CCP & Speedup \\ \midrule
...
Grid size: 128, C = 4 & 237278.48 & \textbf{27886.87} & $\mathbf{8}\times$ \\ \bottomrule
\end{tabular}
...
\centering
  \begin{tabular}{cc}
    \MSHangBox{\includegraphics[width=0.23\textwidth]{images/objectworld/grid_32_obj_30_algo_3_try_2/test_nll_algo_3.pdf}}
    \MSHangBox{\includegraphics[width=0.23\textwidth]{images/objectworld/grid_32_obj_30_algo_3_try_2/evd_num_algo_3.pdf}}
  \end{tabular}
    \caption{Results on ObjectWorld with gridsize of 50 and 2 colors. Left: NLL results on test trajectories. Right: Expected Value Difference results. Each expert trajectory was of length 30.}
...
We use similar DNN architecture for both CCP-IRL and MaxEnt-IRL. Precisely, we use a 2-layer feed-forward network with rectified linear units. We use the Adam \cite{kingma2014adam} optimizer with the initial learning rate set to $10^{-3}$.

First, we compare the computation speed for CCP-IRL and MaxEnt-IRL, Table \ref{table:table_results_objectworld} shows the computation time for both algorithms across differening sized state spaces ($10^3$ to $10^4$). For, smaller state spaces ($10^3$), our method provides a $6\times$ speedup. While for larger state spaces CCP-IRL is $8\times$ faster than MaxEnt-IRL. Thus, as seen in the linear case, CCP-IRL provides a huge computation advantage over MaxEnt-IRL across small and large state spaces. Further, we also note that for more complex MDP problems the computational advantage for CCP-IRL would be much larger. To illustrate this, we run the Objectworld experiment with different discount values, since larger discount values lead to infinite horizon MDP problems which are computationally more expensive. We confirm this in Figure \ref{fig:img_objectworld_maxent_vs_ccp_time_results}, as can be seen in the above figure, with increasing discount values MaxEnt-IRL's computation time increases super-linearly (almost exponentially) while CCP-IRL scales sub-linearly.

We now analyze how the amount of data affects CCP-IRL. Figure \ref{fig:img_objectworld_maxent_vs_ccp_lr_01} plots the NLL and EVD against number of expert demonstrations. Notice that, as observed before, with few expert trajectories (less than 20) all algorithms perform poorly. However, both MaxEnt-IRL and CCP-IRL match expert performance with moderate number of trajectories ($\approx 30$). However, notice that MaxEnt-IRL converges around 20 trajectories while CCP-IRL requires close to 25 trajectories. This larger sample complexity for CCP-IRL is expected given that it requires consistent CCP estimates, however, more importantly we observe that the difference in sample complexity is not large.
In contrast, SC-IRL fails to learn the appropriate parameters in this setting. Although, given more expressive non-linear features, a linear parameterization could suffice for this simple problem, completely non-linear parameterizations for SCIRL have been unexplored. 

In this section, we looked at how CCP-IRL generalizes to parameterized (linear and non-linear) reward functions. In the supplementary material, we further derive how the CCP approach can be used to estimate the reward functions in a \textit{non-parameteric} setting.
...
As noted previously \cite{Ng2000}, the IRL problem is fundamentally under-identified since there maybe infinitely many reward functions consistent with the demonstrated behavior. The CCP framework provides methods to investigate this under-identification problem in IRL. Specifically, the CCP approach clarifies the necessary assumptions in the DDC framework to ensure a ``unique" mapping from trajectories to the reward functions \cite{magnac}. Alternately, in IRL literature, recently \cite{amin2016resolving} identify the causes of this unidentifiability and propose a framework to resolve it. An interesting future direction would be to relax the assumptions proposed by \cite{magnac} and combine these with the framework developed in \cite{amin2016resolving}.

The CCP approach is also used successfully in multi-agent settings where strategic situations generally lead to multiple equilibria. Any IRL algorithm that \emph{solves} the DP numerically will require an ad-hoc equilibrium selection rule. Moreover, there is no data-driven way to ensure the selection rule is identifying the equilibrium in the data. It is therefore highly likely that an IRL algorithm can match the data by selecting the wrong reward parameters and the wrong equilibrium. 
% In other words, matching the data is not enough to guarantee the accuracy of measurements.
However, with CCPs and a dataset on a single path of play, it is possible to estimate rewards consistently.
...
Although, in this work we focus on discrete domains, the CCP approach has been applied to situations with continuous state or action spaces \cite{altuug1998effect}. Alternately, past work in IRL literature has also focused on scaling MaxEnt-IRL to high dimensional continuous spaces. To achieve this, most algorithms use local approximation to estimate the partition function in MaxEnt-IRL. Using local approximations requires giving up on global optimality, but allows the algorithms to scale to high dimensional continuous domains \cite{levine2012continuous, kalakrishnan2013learning, finn2016guided}. Since such approximations are necessary to scale IRL to high dimensional control spaces in robotics, we are currently working on combining the above approaches with our CCP-IRL approach. 

The CCP framework also opens up the possibility of estimating rewards in non-stationary environments. The CCP framework leads to computational simplifications in settings where actions terminate the decision problem or a sequence of actions ``reset" the future state distributions \cite{arcidiacono2014nonstationary}. This allows us to represent future values as arbitrary sequences of future CCPs.
Applying these approximations to non-stationary problems in robotics is an important direction to pursue.
...
% Finally, from the robotics perspective, the process of learning the CCPs is exactly Behavior Cloning (BC), where the policy is learned directly from demonstrations without making the reward function explicit. The CCP-IRL framework can then be interpreted as, an behavior cloning phase followed by a inverse reinforcement learning phase that makes use of a fully specified policy to estimate the reward function.
