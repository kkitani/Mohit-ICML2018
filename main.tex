...
\textcolor{blue}{
To side step computational complexity, past work \cite{klein2012inverse, dvijotham2010inverse} has identified specific classes of problems where the reward function or value function can be solved efficiently. For instance, \cite{klein2012inverse} derives a supervised structured classification framework to estimate the value function by restricting the form of the reward function to be linear. \cite{dvijotham2010inverse} assume a linearly solvable MDP scenario, which allows for efficient computation through the linearization of the value function. While these methods are indeed efficient when key assumptions are met, they may not be applicable to a more general set of problems (\emph{e.g.,} non-linear reward or value functions) that contradict those assumptions.
%
%
...
In contrast, \cite{boularias2011relative} propose using an importance sampling trick to avoid solving the MDP repeatedly. However, this requires sampling from an arbitrary policy which can have large variance. Additionally, \cite{vernaza2012efficient} constrain the features to low dimensional embedding to estimate the partition function effectively. However, this still does not avoid solving the MDP problem repeatedly.
