...
\end{split}    
\end{align}
Again both $\hat{\mathbf{Z}}(a)$ and $\hat{\bm{\epsilon}}(a)$ can be estimated via backwards recursion and is computationally similar to known model-based RL algorithm. Given these values calculating the choice-specific value function or the ex-ante value function for different values of $\theta$ requires simple matrix operations only.

\subsection{Inverse Reinforcement Learning with CCPs}

Maximum Entropy IRL (MaxEnt-IRL) \cite{ziebart2} involves estimating the parameters $\theta$ of the reward function under which a policy $\pi$ has maximum entropy subject to feature matching constraints. 
Solving this problem leads to an iterative procedure in which the ex-ante value function or the choice-specific value function needs to be estimated at every step.
In MaxEnt-IRL we calculate these by solving the RL algorithm using model-based RL.

The above ex-ante value function representations (\eqref{eq:exante_inversion}, \eqref{eq:exante_linear}) can be used to solve the IRL problem efficiently. To see this, notice that in \eqref{eq:exante_inversion} we can pre-calculate the inverse matrix once. Similarly, for linear parameters we can pre-calculate \eqref{eq:choice_specific_Z_tilde}. Given these values we can calculate the ex-ante value function using simple matrix operations.

We now discuss the CCP-IRL algorithm and how it avoids repeatedly solving the original MDP problem \textcolor{blue}{(KK: In order for this statement to make sense, there needs to be a description of MaxEnt IOC or some other form of IRL which makes this point clear. Common feature of your writing is to leave a big chunks of information or assume too much of the reader. Try to put yourself more in the position of the first reader.)}. \textcolor{blue}{(KK: Again, just jumps into explanation without any justification or motivation. This makes for papers that are hard to follow.)}
