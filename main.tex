...
To illustrate the computational efficiency of CCP-IRL we experiment on multiple benchmark problems. We show that CCP-IRL is able to match expert performance with \textit{much less} computational time. Further, as noted before, we also examine the underlying assumptions of consistent CCP estimates. For this, we see how CCP-IRL's performance is affected by the number of expert demonstrations.
...
\noindent{\textbf{Baselines:}} As discussed above, CCP-IRL is most closely related to MaxEnt-IRL, hence we use it as a baseline method to compare our results. Also, we use SCIRL as another baseline.
Since SCIRL also uses expert CCP estimates, although in a max-margin formulation, we use both SCIRL and CCP-IRL to show the efficacy of using CCP estimates in an IRL framework. Further, we also use SCIRL to highlight the generalizability of CCP-IRL to non-linear reward formulations, 

\noindent{\textbf{Metrics:}} For comparative analysis, we use negative log likelihood (NLL) \cite{kitani2012activity} and expected value difference (EVD) \cite{levine2011nonlinear}. NLL is a probabilistic comparison metric and evaluates the likelihood of a path under the predicted policy. For a policy $\pi$, NLL is defined as, $NLL(\pi) = E_{\pi(a|s)}\left[-\log \prod_{t} \pi(a_{t}|s_{t}) \right]$.
...
% As another metric of success, similar to related works \cite{Levine2013, wulfmeier2015maximum}, we use expected value difference (EVD). 
...
\subsection{Highway: Evaluating Linear Rewards} 
...
Similar to previous works \cite{abbeel2004apprenticeship, klein2012inverse}, we use a car driving simulator for linear reward setting. The goal in this environment is to drive on a multi-lane highway without crashing. The agent's car can either move left or right, accelerate, decelerate or keep a constant speed. For the true reward, we penalize off-road driving and collisions while favoring high speed. 

For our reward feature vector, we use a discretization of the state space. In the simplest case, similar to, Klein \emph{et al.} \yrcite{klein2012inverse}, we parameterize the state space using 9 horizontal positions for the user's car, 3 horizontal and 9 vertical positions for the closest traffic's car and 3 speeds. This yields a feature vector of size $729$. For MDPs with large state space, we increase each of the above MDP parameters, such as, car speed, number of lanes \emph{etc.}. However, a one-hot feature representation, as used in \cite{klein2012inverse}, does not scale to such large MDPs. Hence for large MDPs, we group adjacent lanes and closest traffic car's position together, to keep the feature vector representation compact.

First, we verify verify the computational advantage for CCP-IRL across large state spaces ($|S| \in \{10^3, 10^6\}$) in Table~\ref{table:table_results_macro_cells}. CCP-IRL is almost 10$\times$ faster than MaxEnt-IRL, across both large and small state spaces. This gives a significant advantage when running on large spaces, since MaxEnt-IRL is prohibitively expensive in such settings. Also, since SCIRL follows a completely different algorithmic approach to IRL, we do not directly compare the computation time for SCIRL with probabilistic IRL approaches, such as, CCP-IRL and MaxEnt-IRL. However, we do note that in preliminary experiments we found SCIRL to be at least $2\times$ as fast as CCP-IRL.

We also analyze the performance of CCP-IRL given different amounts of expert trajectories.
...
However, with moderate number of trajectories ($\sim$30) all algorithms approach expert behavior.
We also verify CCP-IRL's assumption of consistent CCP estimates in Figure \ref{fig:img_maxent_vs_ccp_gridworld_macro_cell}. As seen above, CCP-IRL requires slightly more number of trajectories to match expert behavior as compared to MaxEnt-IRL. Interestingly, SCIRL requires less data compared to both MaxEnt-IRL and CCP-IRL. We believe this is because SCIRL is solving a supervised classification problem to directly imitate the expert, and such supervised learning provides better learning guarantees than RL based approaches \cite{beygelzimer2011contextual}. However, the low sample complexity of both SCIRL and CCP-IRL reflect the efficacy of CCPs in an IRL setting.
