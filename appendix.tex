...
% (2) the robustness of CCP-IRL under noisy expert demonstrations.
...
% To examine the robustness of CCP-IRL against noisy expert demonstrations we plot the NLL and EVD values in Figure \ref{fig:img_highway_noisy_exp}. As can be seen in the above figure, SCIRL fails to learn the underlying expert reward correctly in this setting. Since the given sub-optimal behavior there exists no optimal beha

% Since, the demonstrated behavior is sub-optimal SCIRL fails to find a reward function that matches demonstrated features. In contrast, MaxEnt-IRL matches the sub-optimal behavior 

% We believe this poor behavior is because there exsts
% poorly because there is no optimal trajectory for any cost function that matches demonstrated features.
